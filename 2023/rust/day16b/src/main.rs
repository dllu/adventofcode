...
fn process(contents: &str) -> u32 {
    ...
    for i in 0..num_cols {
        max_energy = cmp::max(
            max_energy,
            calc_energized(&grid, &Position(0, i as i32), &Direction::Down),
        );
    }
    for i in 0..num_rows {
        max_energy = cmp::max(
            max_energy,
            calc_energized(&grid, &Position(i as i32, 0), &Direction::Right),
        );
    }
    for i in 0..num_rows {
        max_energy = cmp::max(
            ...
            calc_energized(
                &grid,
                &Position(i as i32, num_cols as i32 - 1),
                &Direction::Left,
            ),
        );
    }
    for i in 0..num_cols {
        max_energy = cmp::max(
            ...
            calc_energized(
                &grid,
                &Position(num_rows as i32 - 1, i as i32),
                &Direction::Up,
            ),
        );
    }
